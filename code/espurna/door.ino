...
#define DOOR_MINIMUM_DELAY_CHECK 20
#define DOOR_JSON_BUFFER_LENGTH 256
...
int _buzzerTickerCount, _doorSchHour, _doorSchMin, _doorDelayCheck;
unsigned int _doorState;
time_t _doorStateChangedAt;
bool _openSensorPressed, _closedSensorPressed, _attemptedStop,
    _processingCommand, _stHubDefined;
String _stHubUrl, _stHubToken;
...
void _doorWebSocketOnData(JsonObject &root) {
  ...
  status["closedSensorP"] =
      _closedSensorPressed; // There seems to some limit on the length of keys
...
void _notifySTHub(String content) {
  if (!_stHubDefined) {
    return;
  }

  HTTPClient http;
  // http.begin("http://192.168.1.29:39500/");
  http.begin(_stHubUrl.c_str());
  http.setReuse(true);
  http.setUserAgent(DEVICE_NAME);
  http.addHeader("token", _stHubToken.c_str());
  http.addHeader("Content-Type", "application/json");
  int httpCode = http.POST(content);
  DEBUG_MSG_P(PSTR("[DOOR] _notifySTHub POST %d\n"), httpCode);
  http.end();
}

void _notifySTHub(JsonObject &root) {
  String content;
  root.printTo(content);
  _notifySTHub(content);
}

void _notifySTHub(String key, String value) {
  DynamicJsonBuffer jsonBuffer(DOOR_JSON_BUFFER_LENGTH);
  JsonObject &root = jsonBuffer.createObject();
  root.set(key, value);
  _notifySTHub(root);
}

void _sendDoorStatus() {
  // DEBUG_MSG_P(PSTR("[MAIN] _sendDoorStatus\n"));

  // DynamicJsonBuffer jsonBuffer(DOOR_JSON_BUFFER_LENGTH);
  // JsonObject &root = jsonBuffer.createObject();
  // root["door"] = _formattedDoorState();
  // root["opensensor"] = _openSensorPressed ? "closed" : "open";
  // root["closedsensor"] = _closedSensorPressed ? "closed" : "open";
  _notifySTHub("door", _formattedDoorState());
...
void _sendNotification(const char *msg) {
  ...
  snprintf_P(buffer, sizeof(buffer), PSTR("%s (%02d%02d%02d)"), msg, hour(t),
             minute(t), second(t));

  _notifySTHub("notify", buffer);
...
void _doorConfigure() {
  _doorDelayCheck =
      getSetting("doorDelayCheck", DOOR_MINIMUM_DELAY_CHECK).toInt();
  if (_doorDelayCheck < DOOR_MINIMUM_DELAY_CHECK) {
    _doorDelayCheck = DOOR_MINIMUM_DELAY_CHECK;
  }

  _doorSchHour = getSetting("doorSchHour", 0).toInt();
  _doorSchMin = getSetting("doorSchMin", 0).toInt();
  _stHubUrl = getSetting("doorHubUrl");
  _stHubToken = getSetting("doorHubToken");
  _stHubDefined = strlen(_stHubUrl.c_str()) > 0;
  DEBUG_MSG_P(PSTR("[DOOR] _doorConfigure %d %d %d %s %s\n"), _doorDelayCheck,
              _doorSchHour, _doorSchMin, _stHubUrl.c_str(), _stHubUrl.c_str());
}
...
bool _openSensorEvent() {
  ...
  // if sensor is closed, then door is completely open otherwise door is
  // closing
...
void _verifyOpen() {
  ...
  if (_doorState != DoorState_open) {
    ...
    snprintf_P(buffer, sizeof(buffer),
               PSTR("Door did not open within %d seconds"));
    _sendNotification(buffer);
  }

  _processingCommand = false; // The command should have processed by the
                              // time verification is performed
}

void _openDelayed() { _open(); }
...
bool _open() {
  ...
  if (_doorState == DoorState_open) {
    ...
    if (_attemptedStop) {
      _sendNotification("Door sensor malfunctioning, open sensor "
      ...
      // if door was stopped and it went to opening again (sensor fired
      // too fast), then when _doorOperateTicker fires this function will
      // again set door state to opening.
      _attemptedStop = true;
...
void _verifyClose() {
  ...
  if (_doorState != DoorState_closed) {
    char buffer[40];
    snprintf_P(buffer, sizeof(buffer),
               PSTR("Door did not close within %d seconds"), _doorDelayCheck);
    _sendNotification(buffer);
  }

  _processingCommand = false; // The command should have processed by the time
...
bool _close(bool fromSchedule) {
  ...
  if (_doorState == DoorState_closed) {
    ...
    if (_attemptedStop) {
      _sendNotification("Door sensor malfunctioning, closed sensor "
                        "triggered too quickly");
      return false;
    } else {
      _updateDoorState(DoorState_stopped); // Update and send web notification
      ...
      _doorOperateTicker.once_ms(DOOR_RELAY_PULSE_DELAY + 1000, _closeDelayed,
                                 fromSchedule);
    }
  } else {
    _attemptedStop = false;
...
void _doorSetupAPI() {
  apiRegister(
      ...
      [](char *buffer, size_t len) {
        // Send critical details through API since the bridge may prevent
        // message from being sent if the value did not change
        snprintf_P(buffer, len, PSTR("%d/%d/%d"), _doorState,
                   _openSensorPressed ? 1 : 0, _closedSensorPressed ? 1 : 0);
      },
      [](const char *payload) {
  ...
  apiRegister(
      DOOR_API_ST_HUB,
      [](char *buffer, size_t len) { snprintf_P(buffer, len, PSTR("OK")); },
      [](const char *payload) {
        if (strlen(payload) > 0) {
          ...
          for (int i = 0; tok != NULL; tok = strtok(NULL, ","), i++) {
            switch (i) {
            case 0: // ipaddress
              _stHubUrl = "http://" + String(tok) + ":";
              break;
            case 1: // port
              _stHubUrl.concat(tok);
              _stHubUrl.concat("/");
              setSetting("doorHubUrl", _stHubUrl);
              _stHubDefined = true;
              break;
            case 2: // token
              _stHubToken = String(tok);
              setSetting("doorHubToken", _stHubToken);
              break;
            }
          }

          DEBUG_MSG_P(PSTR("[DOOR] _doorSetupAPI %s %s\n"), _stHubUrl.c_str(),
                      _stHubToken.c_str());
        }
      });
}

// Called from espurna.setup
void doorSetup() {
  // mode=BUTTON_SWITCH -> EVENT_CHANGED -> BUTTON_EVENT_CLICK
  // mode=BUTTON_PUSHBUTTON -> EVENT_PRESSED/EVENT_RELEASED ->
  // BUTTON_EVENT_PRESSED/(BUTTON_EVENT_CLICK or BUTTON_EVENT_LNGCLICK or
  // BUTTON_EVENT_LNGLNGCLICK)
  ...
  _openSensor = new DebounceEvent(DOOR_OPEN_SENSOR_PIN,
                                  BUTTON_SWITCH | DOOR_OPEN_SENSOR_PULLUP);
  _closedSensor = new DebounceEvent(DOOR_CLOSED_SENSOR_PIN,
                                    BUTTON_SWITCH | DOOR_CLOSED_SENSOR_PULLUP);
...
// Websocket Callbacks
  ...
  _doorSetupAPI(); // API_SUPPORT is required
  _sendDoorStatus();
