...
bool _openSensorPressed, _closedSensorPressed, _attemptedStop, _processingCommand, _stHubDefined;
...
bool _doorWebSocketOnKeyCheck(const char *key, JsonVariant &value) { return (strncmp(key, "door", 4) == 0); }
...
void _doorWebSocketOnData(JsonObject &root) {
    ...
    status["closedSensorP"] = _closedSensorPressed; // There seems to a limit on the length of key
...
void _doorConfigure() {
    _doorDelayCheck = getSetting("doorDelayCheck", DOOR_MINIMUM_DELAY_CHECK).toInt();
...
bool _closedSensorEvent() {
    ...
    DEBUG_MSG_P(PSTR("[DOOR] Closed sensor, %s, doorState=%u %s\n"), _closedSensorPressed ? "closed" : "open",
                _doorState, _formattedDoorState());
    _sendDoorStatus();
    return true;
}
...
void _doorSetupAPI() {
    apiRegister(
        ...
        [](char *buffer, size_t len) {
            ...
            snprintf_P(buffer, len, PSTR("%d/%d/%d"), _doorState, _openSensorPressed ? 1 : 0,
                       _closedSensorPressed ? 1 : 0);
        },
        [](const char *payload) {
